...
from tensorflow.keras.layers import RNN, GRUCell, LSTMCell, Dense, LayerNormalization
...
def tgan (dataX, parameters):
    ...
    def rnn_cell(module_name, hidden_dim):
        if module_name == 'gru':
            return GRUCell(hidden_dim, activation='tanh')
        elif module_name == 'lstm':
            return LSTMCell(hidden_dim, activation='tanh')
        elif module_name == 'lstmLN':
            # Custom Layer Normalized LSTM
            return tf.keras.layers.RNN(
                [LSTMCell(hidden_dim, activation='tanh'), LayerNormalization()],
    ...
    def embedder(X, T):
        with tf.compat.v1.variable_scope("embedder", reuse=tf.compat.v1.AUTO_REUSE):
            rnn_layers = [rnn_cell(module_name, hidden_dim) for _ in range(num_layers)]
            stacked_rnn = RNN(rnn_layers, return_sequences=True)
            e_outputs = stacked_rnn(X, mask=tf.sequence_mask(T))
            dense_layer = Dense(hidden_dim, activation='sigmoid')
    ...
    def recovery(H, T):
        with tf.compat.v1.variable_scope("recovery", reuse=tf.compat.v1.AUTO_REUSE):
            rnn_layers = [rnn_cell(module_name, hidden_dim) for _ in range(num_layers)]
            stacked_rnn = RNN(rnn_layers, return_sequences=True)
            r_outputs = stacked_rnn(H, mask=tf.sequence_mask(T))
            dense_layer = Dense(data_dim, activation='sigmoid')
    ...
    def generator(Z, T):
        with tf.compat.v1.variable_scope("generator", reuse=tf.compat.v1.AUTO_REUSE):
            rnn_layers = [rnn_cell(module_name, hidden_dim) for _ in range(num_layers)]
            stacked_rnn = RNN(rnn_layers, return_sequences=True)
            e_outputs = stacked_rnn(Z, mask=tf.sequence_mask(T))
            dense_layer = Dense(hidden_dim, activation='sigmoid')
    ...
    def supervisor(H, T):
        with tf.compat.v1.variable_scope("supervisor", reuse=tf.compat.v1.AUTO_REUSE):
            rnn_layers = [rnn_cell(module_name, hidden_dim) for _ in range(num_layers - 1)]
            stacked_rnn = RNN(rnn_layers, return_sequences=True)
            e_outputs = stacked_rnn(H, mask=tf.sequence_mask(T))
            dense_layer = Dense(hidden_dim, activation='sigmoid')
    ...
    def discriminator(H, T):
        with tf.compat.v1.variable_scope("discriminator", reuse=tf.compat.v1.AUTO_REUSE):
            rnn_layers = [rnn_cell(module_name, hidden_dim) for _ in range(num_layers)]
            stacked_rnn = RNN(rnn_layers, return_sequences=True)
            d_outputs = stacked_rnn(H, mask=tf.sequence_mask(T))
            dense_layer = Dense(1, activation=None)
    ...
    dataX_ = np.array(dataX)
    dataT_ = np.array(dataT)
    import time
    time1 = time.time()
    ...
    for itt in range(iterations):
        ...
        # Batch setting
        train_idx = np.random.choice(No, batch_size, replace=False)
        # was converted to list before
        X_mb = dataX_[train_idx]
        T_mb = dataT_[train_idx]
            
        # Train embedder        
        _, step_e_loss = sess.run([E0_solver, E_loss_T0], feed_dict={X: X_mb, T: T_mb})
        
        if itt % 1000 == 0:
            print("step_e_loss:", step_e_loss, "type:", type(step_e_loss))
